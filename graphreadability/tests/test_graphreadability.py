...
class TestGraphReadability(unittest.TestCase):
    ...
    def test_dataframe_generation(self):
        ...
        tables = pd.DataFrame.from_records(metric_tables, index=self.graph_names, columns=metric_table.index).sort_values(by="Combined", ascending=False)
        self.assertTrue(len(tables) == len(self.graphs), "Mismatch in number of graphs and metric tables")
