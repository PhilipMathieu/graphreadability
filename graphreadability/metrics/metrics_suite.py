...
class MetricsSuite:
    ...
    def __init__(
        ...
        self.metrics = {
            "edge_crossing": {
                "func": self.edge_crossing,
                "value": None,
                "num_crossings": None,
                "weight": 0,
            },
            "edge_orthogonality": {
                "func": self.edge_orthogonality,
                "value": None,
                "weight": 0,
            },
            "node_orthogonality": {
                "func": self.node_orthogonality,
                "value": None,
                "weight": 0,
            },
            "angular_resolution": {
                "func": self.angular_resolution,
                "value": None,
                "weight": 0,
            },
            "symmetry": {"func": self.symmetry, "value": None, "weight": 0},
            "node_resolution": {
                "func": self.node_resolution,
                "value": None,
                "weight": 0,
            },
            ...
            "neighbourhood_preservation": {
                "func": self.neighbourhood_preservation,
                "value": None,
                "weight": 0,
            },
            "aspect_ratio": {"func": self.aspect_ratio, "value": None, "weight": 0},
            "node_uniformity": {
                "func": self.node_uniformity,
                "value": None,
                "weight": 0,
            },
        }
        ...
        if type(sym_tolerance) != int and type(sym_tolerance) != float:
            raise TypeError(
                f"sym_tolerance must be a number, not {type(sym_tolerance)}"
            )
    ...
    def count_crossings(self, G=None):
        ...
        for e in G.edges:
            ...
            a_p1 = (
                G.nodes[e[0]]["x"],
                G.nodes[e[0]]["y"],
            )  # Position of source node of e
            a_p2 = (
                G.nodes[e[1]]["x"],
                G.nodes[e[1]]["y"],
            )  # Position of target node of e
            line_a = (a_p1, a_p2)
    ...
    def angular_resolution(self, all_nodes=False):
        ...
        # Return 1 minus the average of minimum angles
        return (
            1 - (angles_sum / self.graph.number_of_nodes())
            if all_nodes
    ...
    def node_orthogonality(self):
        ...
        min_x, min_y = (
            self.graph.nodes[first_node]["x"],
            self.graph.nodes[first_node]["y"],
        )
        ...
        for node in self.graph.nodes:
            self.graph.nodes[node]["x"] = (
                float(self.graph.nodes[node]["x"]) - x_distance
            )
            self.graph.nodes[node]["y"] = (
                float(self.graph.nodes[node]["y"]) - y_distance
            )
    ...
    def _sym_value(self, e1, e2, G):
        ...
        if self._is_minor(P, G) == self._is_minor(X, G) and self._is_minor(
            Q, G
        ) == self._is_minor(Y, G):
            # P=X and Q=Y
            return 1
        elif self._is_minor(P, G) == self._is_minor(Y, G) and self._is_minor(
            Q, G
        ) == self._is_minor(X, G):
            # P=Y and X=Q
            return 1
        elif self._is_minor(P, G) == self._is_minor(X, G) and self._is_minor(
            Q, G
        ) != self._is_minor(Y, G):
            # P=X but Q!=Y
            return 0.5
    ...
    def edge_length(self, ideal=None):
        ...
        for edge in self.graph.edges:
            ...
            b = self.graph.nodes[edge[1]]["x"], self.graph.nodes[edge[1]]["y"]
            edge_length_sum += (
                abs(ideal_edge_length - self._euclidean_distance(a, b))
                / ideal_edge_length
            )
    ...
    def get_stress(self):
        ...
        X = np.array(
            [[float(G.nodes[n]["x"]), float(G.nodes[n]["y"])] for n in G.nodes()]
        )
        ...
        diff = np.sqrt(
            abs(ss.reshape((N, 1)) + ss.reshape((1, N)) - 2 * np.dot(X, X.T))
        )
    ...
    def stress_not_normal(self, G):
        ...
        X = np.array(
            [
                [float(self.graph.nodes[n]["x"]), float(self.graph.nodes[n]["y"])]
        ...
        for i in range(num_cells):
            for j in range(num_cells):
                for point in points:
                    ...
                    # print(square)
                    if self._is_point_inside_square(
                        point,
